...
import elcon.mods.agecraft.core.tech.old.TechTreeOld;
import elcon.mods.agecraft.core.tech.old.TechTreeClientOld;
import elcon.mods.agecraft.core.tech.old.TechTreeComponentOld;

@Deprecated
public class GuiTechTreeOld extends GuiScreen {

	private static final int guiMapTop = TechTreeOld.minDisplayColumn * 24 - 112;
	private static final int guiMapLeft = TechTreeOld.minDisplayRow * 24 - 112;
	private static final int guiMapBottom = TechTreeOld.maxDisplayColumn * 24 - 77;
	private static final int guiMapRight = TechTreeOld.maxDisplayRow * 24 - 77;
	...
	private LinkedList<TechTreeComponentOld> techTreeComponents = new LinkedList<TechTreeComponentOld>();
	public static List unlockedTechTreeComponents = new ArrayList();
	private FontRenderer galFontRenderer;
	private TechTreeComponentOld currentHighlight = null;

	public GuiTechTreeOld() {
		short var2 = 141;
		short var3 = 141;
		...
		Collection col = TechTreeOld.techTreeComponents.values();
		for(Iterator i = col.iterator(); i.hasNext();) {
			Object t = i.next();
			techTreeComponents.add((TechTreeComponentOld) t);
		}
		for(String s : TechTreeClientOld.unlockedTechComponents) {
			unlockedTechTreeComponents.add(s);
		}
		galFontRenderer = Minecraft.getMinecraft().fontRenderer;
	}

	public GuiTechTreeOld(double x, double y) {
		...
		for(Iterator i = col.iterator(); i.hasNext();) {
			Object res = i.next();
			techTreeComponents.add((TechTreeComponentOld) res);
		}
		for(String s : TechTreeClientOld.unlockedTechComponents) {
			unlockedTechTreeComponents.add(s);
		}
	...
	protected void genBackground(int par1, int par2, float par3) {
		...
		for(int var22 = 0; var22 < techTreeComponents.size(); var22++) {
			TechTreeComponentOld var33 = (TechTreeComponentOld) techTreeComponents.get(var22);
		...
		for(var24 = 0; var24 < techTreeComponents.size(); var24++) {
			TechTreeComponentOld var35 = (TechTreeComponentOld) techTreeComponents.get(var24);
	...
	private boolean canUnlockResearch(TechTreeComponentOld res) {
		if((res.parents != null) && (res.parents.length > 0)) {
			for(TechTreeComponentOld parent : res.parents) {
