	...
	public void registerIcons(IconRegister iconRegister) {
		for(int i = 0; i < ToolRegistry.toolMaterials.length; i++) {
			...
			if(toolMaterial != null) {
				iconsHead[i][0] = iconRegister.registerIcon("agecraft:tools/arrow/" + toolMaterial.name + "/arrow" + ElConCore.firstUpperCase(toolMaterial.name));
				for(int j = 0; j < 3; j++) {
					iconsHead[i][j + 1] = iconRegister.registerIcon("agecraft:tools/arrow/" + toolMaterial.name + "/arrow" + ElConCore.firstUpperCase(toolMaterial.name) + Integer.toString(j));
				}
			}
		}
		for(int i = 0; i < ToolRegistry.toolRodMaterials.length; i++) {
			...
			if(toolRodMaterial != null) {
				iconsRod[i][0] = iconRegister.registerIcon("agecraft:tools/sticks/arrow/" + toolRodMaterial.name + "/arrow" + ElConCore.firstUpperCase(toolRodMaterial.name));
				for(int j = 0; j < 3; j++) {
					iconsRod[i][j + 1] = iconRegister.registerIcon("agecraft:tools/sticks/arrow/" + toolRodMaterial.name + "/arrow" + ElConCore.firstUpperCase(toolRodMaterial.name) + Integer.toString(j));
				}
			}
		}
	}
