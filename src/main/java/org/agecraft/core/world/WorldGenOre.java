...
public class WorldGenOre extends WorldGenerator {
	...
	public WorldGenOre(Block minableBlock, int minableBlockMeta, boolean extendedMetadata, int numberOfBlocks, Block blockToReplace, int oreGenPerChunk, int oreGenMinY, int oreGenMaxY) {
		this(minableBlock, numberOfBlocks, blockToReplace, oreGenPerChunk, oreGenMinY, oreGenMaxY);
					...
					for(int yy = startY; yy <= endY; ++yy) {
						...
						if(d12 * d12 + d13 * d13 < 1.0D) {
							for(int zz = startZ; zz <= endZ; ++zz) {
								...
								if(d12 * d12 + d13 * d13 + d14 * d14 < 1.0D && (block != null && block.isReplaceableOreGen(world, xx, yy, zz, blockToReplace))) {
									if(extendedMetadata) {
										world.setBlock(xx, yy, zz, minableBlock, 0, 2);
										TileEntity tile = minableBlock.createTileEntity(world, 0);
										if(tile instanceof TileEntityMetadata) {
											((TileEntityMetadata) tile).setTileMetadata(minableBlockMeta);
											world.setTileEntity(xx, yy, zz, tile);
										}
									} else {
