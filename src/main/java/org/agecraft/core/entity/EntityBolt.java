...
public class EntityBolt extends Entity implements IProjectile {
	...
	public EntityBolt(World world, EntityLivingBase entityShooter, EntityLivingBase entity, float f1, float f2) {
		this(world);
	...
	public EntityBolt(World world, EntityLivingBase entityShooter, float par3) {
		this(world);
		shootingEntity = entityShooter;
		if(entityShooter instanceof EntityPlayer) {
			canBePickedUp = 1;
	...
	public void onUpdate() {
		...
		if(inGround) {
			Block blockInGround = worldObj.getBlock(tileX, tileY, tileZ);
			int k = worldObj.getBlockMetadata(tileX, tileY, tileZ);
			if(blockInGround == inTile && k == inData) {
				ticksInGround++;
			...
			if(movingobjectposition != null) {
				if(movingobjectposition.entityHit != null) {
					...
					inTile = worldObj.getBlock(tileX, tileY, tileZ);
					...
					setIsCritical(false);
					if(inTile != null) {
						inTile.onEntityCollidedWithBlock(worldObj, tileX, tileY, tileZ, this);
					}
				}
			}
			if(getIsCritical()) {
	...
	public ItemStack getItemStack() {
		ItemStack stack = new ItemStack(Tools.bolt, 1, 0);
		NBTTagCompound nbt = new NBTTagCompound();
		NBTTagCompound tag = new NBTTagCompound();
		tag.setInteger("Type", 20);
	...
	public void onCollideWithPlayer(EntityPlayer player) {
		if(!worldObj.isRemote && inGround && boltShake <= 0) {
	...
	protected void readEntityFromNBT(NBTTagCompound nbt) {
		...
		boltShake = nbt.getByte("BoltShake") & 255;
	...
	protected void writeEntityToNBT(NBTTagCompound nbt) {
		...
		nbt.setByte("BoltShake", (byte) boltShake);
