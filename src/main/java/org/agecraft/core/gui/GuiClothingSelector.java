...
import org.agecraft.core.clothing.Clothing;
...
import org.agecraft.core.clothing.ClothingRegistry.ClothingTypeIndexComparator;
...
import elcon.mods.elconqore.gui.GuiButton;
import elcon.mods.elconqore.gui.GuiButtonDefault;
import elcon.mods.elconqore.gui.GuiButtonListVertical;
...
public class GuiClothingSelector extends GuiScreen {
	...
	public GuiClothingSelector(List<String> clothingTypesChangable) {
		...
		AgeCraftCore.instance.packetHandler.sendToServer(new MessageClothingUnlockRequest(mc.thePlayer.getCommandSenderName(), mc.theWorld.provider.dimensionId));
	}
	...
	public void initGui() {
		...
		playerClothing = PlayerClothingClient.getPlayerClothing(mc.thePlayer.getCommandSenderName());
		...
		PlayerClothing clothing = PlayerClothingClient.getPlayerClothing(mc.thePlayer.getCommandSenderName()).copy();
		clothing.player = mc.thePlayer.getCommandSenderName() + "-Temp";
		PlayerClothingClient.addPlayerClothing(clothing);
		clothing = PlayerClothingClient.getPlayerClothing(mc.thePlayer.getCommandSenderName()).copy();
		clothing.player = mc.thePlayer.getCommandSenderName() + "-TempPiece";
		...
		buttonList.clear();
		buttonList.add(new GuiButton(0, guiLeft + 100, guiTop + 146, 0, 182, 16, 16, AgeCraftCoreClient.guiClothingSelector, ""));
		buttonList.add(new GuiButton(1, guiLeft + 120, guiTop + 146, 32, 182, 16, 16, AgeCraftCoreClient.guiClothingSelector, ""));
		buttonList.add(new GuiButton(2, guiLeft + 140, guiTop + 146, 16, 182, 16, 16, AgeCraftCoreClient.guiClothingSelector, ""));
		...
		for(ClothingType type : clothingTypes) {
			if(type != null && clothingTypesChangeable.contains(type.name)) {
				buttonsClothingType.addButton(new GuiButtonClothingType(5 + index, guiLeft + 10, guiTop + 16 + index * 16, 0, 166, 68, 16, AgeCraftCoreClient.guiClothingSelector, LanguageManager.getLocalization("clothing.type." + type.name), type.name, index));
				index++;
		...
		for(int j = 0; j < 2; j++) {
			for(int i = 0; i < 8; i++) {
				buttonList.add(new GuiButtonClothingColor(27 + i + j * 8, guiLeft + 183 + i * 8, guiTop + 146 + j * 8, 68 + i * 8, 166 + j * 8, 8, 8, AgeCraftCoreClient.guiClothingSelector, ""));
			}
		}
		...
		for(int i = 0; i < clothingCategories.size(); i++) {
			GuiToggleButton button = new GuiToggleButton(100 + i, guiLeft + 267, guiTop + 19 + i * 16, 140, 166, 82, 16, AgeCraftCoreClient.guiClothingSelector, LanguageManager.getLocalization("clothing.category." + clothingCategories.get(i).name), true);
	...
	public void updateClothingList() {
		...
		if(clothingPiecesList != null) {
			for(int i = 0; i < clothingPieces.size(); i++) {
				...
				buttonsClothingPieces.addButton(new GuiToggleButton(1000 + i, guiLeft + 267, guiTop + 19 + i * 16, 140, 166, 82, 16, AgeCraftCoreClient.guiClothingSelector, LanguageManager.getLocalization("clothing." + clothing.category.name + "." + clothing.type.name + "." + clothing.name), true));
			}
		}
	...
	public void updateTempPieceClothing() {
		...
		for(int i = 0; i < 16; i++) {
			...
			((GuiToggleButton) buttonList.get(11 + clothingTypesChangeableCount + i)).visible = flag;
		...
		PlayerClothing clothing = PlayerClothingClient.getPlayerClothing(mc.thePlayer.getCommandSenderName() + "-TempPiece");
		...
		clothing.clothingTypeExclusive = clothingTypes.get(currentClothingType).name;
		clothing.clothingColorExclusive = currentClothingColor;
		PlayerClothingClient.updatePlayerClothing(mc.thePlayer.getCommandSenderName() + "-TempPiece");
	}
	...
	protected void actionPerformed(net.minecraft.client.gui.GuiButton button) {
		if(button.id == 0) {
			...
			AgeCraftCore.instance.packetHandler.sendToServer(new MessageClothingSelector(mc.thePlayer.getCommandSenderName(), clothingPiecesCart));
			mc.thePlayer.closeScreen();
	...
	public void drawScreen(int mouseX, int mouseY, float partialTickTime) {
		...
		mc.getTextureManager().bindTexture(AgeCraftCoreClient.guiClothingSelector);
		drawTexturedModalRect(guiLeft, guiTop, 0, 0, xSize, ySize);
		mc.getTextureManager().bindTexture(AgeCraftCoreClient.guiClothingSelectorSideBar);
		if(showLeftList) {
			...
			if(rightList == 0) {
				...
				for(net.minecraft.client.gui.GuiButton button : buttonsClothingCategories.buttons) {
					...
					if(!isClothingCategoryEnabled[button.id - 100]) {
						...
						drawTexturedModelRectFromIcon(button.xPosition + 68, button.yPosition, AgeCraftCoreClient.iconLock, 16, 16);
					}
				}
			} else if(rightList == 1) {
