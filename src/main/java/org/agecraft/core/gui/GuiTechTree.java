...
import org.agecraft.core.techtree.TechTreeClient;
...
public class GuiTechTree extends GuiScreen {
	...
	public void initGui() {
		buttonList.clear();
		buttonList.add(new GuiButton(1, width / 2 + 24, height / 2 + 74, 80, 20, LanguageManager.getLocalization("gui.done")));
		pageButton = new GuiButton(2, (width - paneWidth) / 2 + 24, height / 2 + 74, 125, 20, LanguageManager.getLocalization("techtree." + pageName + ".name"));
	...
	public void drawScreen(int mx, int my, float par3) {
		...
		genTechTreeBackground(mx, my, par3);
	...
	protected void drawTitle() {
		...
		fontRendererObj.drawString(LanguageManager.getLocalization("techtree.name"), i + 15, j + 5, 4210752);
	}

	protected void genTechTreeBackground(int par1, int par2, float par3) {
		...
		for(TechTreeComponent component : components) {
			...
			if(j4 >= -24 && l3 >= -24 && j4 <= 224 && l3 <= 155) {
				...
				mc.getTextureManager().bindTexture(AgeCraftCoreClient.guiTechTree);
				i5 = k1 + j4;
				l4 = l1 + l3;
				...
				if(component.stack != null) {
					...
					mc.renderEngine.bindTexture(AgeCraftCoreClient.guiTechTreeIcons);
		...
		if(currentHighlight != null) {
			String s = currentHighlight.getName();
			...
			if(TechTreeClient.canUnlockComponent(currentHighlight.pageName, currentHighlight.name)) {
				i5 = Math.max(fontRendererObj.getStringWidth(s), 120);
				l4 = fontRendererObj.splitStringWidth(s1, i5);
				...
				fontRendererObj.drawSplitString(s1, j4, l3 + 12, i5, -6250336);
				if(TechTreeClient.hasUnlockedComponent(currentHighlight.pageName, currentHighlight.name)) {
					fontRendererObj.drawStringWithShadow(LanguageManager.getLocalization("techtree.unlocked"), j4, l3 + l4 + 4, -7302913);
				}
			} else {
				i5 = Math.max(fontRendererObj.getStringWidth(s), 120);
				i4 = fontRendererObj.splitStringWidth(LanguageManager.getLocalization("techtree.locked"), i5);
				drawGradientRect(j4 - 3, l3 - 3, j4 + i5 + 3, l3 + i4 + 12 + 3, -1073741824, -1073741824);
				fontRendererObj.drawSplitString(LanguageManager.getLocalization("techtree.locked"), j4, l3 + 12, i5, -9416624);
			}
			fontRendererObj.drawStringWithShadow(s, j4, l3, TechTreeClient.canUnlockComponent(currentHighlight.pageName, currentHighlight.name) ? (currentHighlight.isSpecial ? -128 : -1) : (currentHighlight.isSpecial ? -8355776 : -8355712));
		}
		GL11.glEnable(GL11.GL_DEPTH_TEST);
		GL11.glEnable(GL11.GL_LIGHTING);
